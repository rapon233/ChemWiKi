...
4. 离心泵的吸入特性采用流量-允许汽蚀余量($Q-\Delta h$)特性(例如离心油泵)或流量-允许吸上真空高度($Q-\text{NPSH}$)特性(例如凝水泵)而旋涡泵采用流量-允许吸上真空高度特性。通常，离心泵的流量允许吸上真空高度特性值较旋涡泵的特性值高(因为旋涡泵入口损失较大)。      
...
两台相似泵的性能曲线是相似或类似的，通常可采用比转数反映这一特征，比转数$n_s$用下式表示：   
...
* 泵的比转数大小也反映了泵性能的特点。低比转数离心泵泵的Q-H特性较平坦而且为驼峰特性(有不稳定区域,工作不如陡降特性稳定);高比转数离心泵的Q-H特性较陡。    
